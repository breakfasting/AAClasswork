...
function sumArray(array) {
    ...
    return sumArray(array.slice(0, array.length - 1)) + array.pop();
}
...
function fileFinder(directories, targetFile) {
    if (targetFile in directories) return true;

    for (directory in directories) {
        if (directory[0] === '/') {
            if (fileFinder(directories[directory], targetFile)) return true
        }
    }
    return false
}
...
function pathFinder(directories, targetFile) {
    if (targetFile in directories) return `/${targetFile}`

    for (let file in directories) {
        if (file[0] === '/') {
            let find = pathFinder(directories[file], targetFile);
            if (find) {
                return `${file}${find}`
            }
        }
