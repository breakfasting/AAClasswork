...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT artist, COUNT(DISTINCT album)
    FROM albums
...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT style, SUM(price) / SUM(subq.count)
  FROM albums
  JOIN styles ON styles.album = albums.asin
  JOIN (
    SELECT asin, COUNT(tracks.*) AS count
    FROM albums
    JOIN tracks ON tracks.album = albums.asin
    WHERE price IS NOT NULL
    GROUP BY asin
  ) AS subq ON albums.asin = subq.asin
  GROUP BY style
  ORDER BY SUM(price) / SUM(subq.count) DESC, style
  LIMIT 5
  SQL
end
